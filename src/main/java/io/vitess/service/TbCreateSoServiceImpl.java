...
//		private static final Long[] WlB_SHOP = {6016L, 6111L, 6946L, 9241L, 9240L};
    ...
    public MqSoLog convertTradeToMqSo(com.taobao.api.domain.Trade tbTrade, Long shopId) throws Exception {
        ...
        for (MqSoLineLog slLog : slLogs) {
            ...
            if(mqSoPackingInfoLog != null){
                mqSoPackingInfoLog.setSoLineLogId(slLog.getId());
                mqSoPackingInfoLog.setSoLineLogId(soLog.getId());
        ...
        // insert delivery
        deliveryInfo.setSoLogId(soLog.getId());
    ...
    private void constructMqSo(com.taobao.api.domain.Trade tbTrade, MqSoLog soLog){
        ...
        if(StringUtils.isNotBlank(tbTrade.getOmnichannelParam())){
            soLog.setOmnichannelParam(StringUtils.substring(tbTrade.getOmnichannelParam(), 0, 1000));
        }
    ...
    private void constructMqService(com.taobao.api.domain.Trade tbTrade, MqSoLog soLog, List<MqSoServiceLineLog> serviceOrderLineList){
        ...
        //商城虚拟服务
        List<com.taobao.api.domain.ServiceOrder> tbServiceOrders = tbTrade.getServiceOrders();
...
//			memLog.setSoLog(soLog.getId());
    ...
    private void constructMqLine(com.taobao.api.domain.Trade tbTrade, MqSoLog soLog, List<MqSoLineLog> slLogs, CompanyShop shopInfo){
        List<com.taobao.api.domain.Order> tbLines = tbTrade.getOrders(); // 平台订单明细行消息信息
        ...
        for (com.taobao.api.domain.Order tbLine: tbLines) {
            ...
            if(isBundle && AppleConstants.APPLE_SHOP_ID.equals(soLog.getShopId())){
                if(!StringUtils.isNotBlank(tbLine.getAssemblyRela())){
                    continue;
                }
            ...
            // 整单分摊到行上的折扣
            soLineLog.setPartMjzDiscount(StringUtils.isNotBlank(tbLine.getPartMjzDiscount())? new BigDecimal(tbLine.getPartMjzDiscount()): BigDecimal.ZERO);
            // 平台行应付金额: (商品价格*商品数量 +手工调整金额 -子订单级订单优惠金额)
            soLineLog.setTotalFee(StringUtils.isNotBlank(tbLine.getTotalFee())? new BigDecimal(tbLine.getTotalFee()): BigDecimal.ZERO);
...
//			soLineLog.setUnitPrice(new BigDecimal(tbLine.getTotalFee()).divide(new BigDecimal(tbLine.getNum()), BigDecimal.ROUND_HALF_UP, 2));
        ...
        if(StringUtils.isNotBlank(cusTex)){
...
//		if(platformTypeFlag[3]){																			                                                    // cj 集货订单
    ...
    private void packageInfo(com.taobao.api.domain.Order tbLine, MqSoLog soLog, MqSoLineLog soLineLog){
        ...
        if (StringUtils.isNotBlank(duihuanCity) && duihuanCity.contains("兑换城市")) {
            ...
            packingInfo.setSoLogId(soLog.getId());
            packingInfo.setSoLineLogId(soLineLog.getId());
            // 行包装保存
            soLineLog.setMqSoPackingInfoLog(packingInfo);
//		mqSoPackingInfoLogDao.insert(packingInfo);
        }
    }
    ...
    private void convertTradeToMqSoPromotion(List<MqSoLineLog> mqSoLineLogList, MqSoLog soLog, Trade tbTrade) {
        List<com.taobao.api.domain.PromotionDetail> tbPromotionDetails = tbTrade.getPromotionDetails(); // 平台金额满减消息信息
        ...
        for (com.taobao.api.domain.PromotionDetail proMsg : tbPromotionDetails) {
            ...
            Integer itemNum = 0;
            if (StringUtils.isNotBlank(proMsg.getGiftItemNum())) {
